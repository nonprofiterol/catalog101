...
class ControllerExtensionModulePavreassurance extends Controller {
	...
	// 	echo '<pre>'.print_r( $this->request->post,1 );die; 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if( isset($this->request->get['module_id']) ) {
				$this->response->redirect($this->url->link('extension/module/pavreassurance', 'module_id='.$this->request->get['module_id'].'&token=' . $this->session->data['token'], 'SSL'));
			}else{
				$this->response->redirect($this->url->link('extension/module/pavreassurance', 'token=' . $this->session->data['token'], 'SSL'));
			}
		}
		...
		if (isset($this->request->get['module_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			...
			$this->data['action'] = $this->url->link('extension/module/pavreassurance', 'module_id='.$this->request->get['module_id'].'&token=' . $this->session->data['token'], 'SSL');
		}else {
			$module_info = $default;
			$this->data['selectedid'] = 0;
			$this->data['subheading'] = $this->language->get('text_create_new_module');
			$this->data['action'] = $this->url->link('extension/module/pavreassurance', 'token=' . $this->session->data['token'], 'SSL');
		}
		...
		$moduletabs = $this->model_extension_module->getModulesByCode( 'pavreassurance' );
	 	$this->data['link'] = $this->url->link('extension/module/pavreassurance', 'token=' . $this->session->data['token'] . '', 'SSL');
		...
		$this->response->setOutput($this->load->view('extension/module/pavreassurance.tpl', $this->data));
	}
	...
	public function _breadcrumbs(){
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/module/pavreassurance', 'token=' . $this->session->data['token'], 'SSL'),
			'separator' => ' :: '
		);
	...
	public function getStore(){
		...
		foreach ($stores as &$store) {
			...
			$store['option'] = $this->url->link('extension/module/pavreassurance', $url.'&token=' . $this->session->data['token'], 'SSL');
		}
